defmodule Elixademy.Accounts.UserImpersonation do
  use Ash.Resource,
    otp_app: :Elixademy,
  ...
  actions do
    ...
    action :stop_impersonation, :map do
      ...
      run Elixademy.Accounts.UserImpersonation.Actions.StopImpersonation
    end
  end
  ...
  relationships do
    belongs_to :impersonator, Elixademy.Accounts.User do
      source_attribute :impersonator_user_id
      description "The user who is performing the impersonation."
    end

    belongs_to :impersonated, Elixademy.Accounts.User do
