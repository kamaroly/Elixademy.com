defmodule Elixademy.Accounts.TeamTest do
  ...
  describe "Team tests" do
    test "User team can be created" do
      ...
      team_count = Ash.count!(Elixademy.Accounts.Team) + 1
      team_attrs = %{name: "Team #{team_count} #{uuid}", domain: "team_#{team_count}_#{uuid}"}

      {:ok, team} = Ash.create(Elixademy.Accounts.Team, team_attrs, actor: user)

      # New team should be stored successfully.
      assert Elixademy.Accounts.Team
      ...
      assert Elixademy.Accounts.User
